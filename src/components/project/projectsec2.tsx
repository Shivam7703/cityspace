...
export default function Projectsec2() {
  ...
  useEffect(() => {
    const updateLineHeight = () => {
      ...
      lineRef.current.style.height = `${scrollProgress * (height - 70)}px`;
    };
  ...
  return (
    <section  className="p-6 sm:p-16 bg-blue1">
      ...
      <div className="relative gap-y-8 py-7 md:py-12 flex flex-wrap justify-between overflow-hidden" ref={sectionRef}>
      <div
  ref={lineRef}
  className="absolute left-1/2 z-20 -translate-x-1/2 w-2 top-3 transition-all duration-300"
>
  <div
    className="w-full h-full border-l-8 border-dotted  border-white"
  />
  <IoMdArrowDropdownCircle className="text-brown2 text-xl bg-white mt-2 rounded-full -ml-[6px]" />
</div>


<div
          className="absolute left-1/2 z-10 -translate-x-1/2 bg-brown2 h-[94%] w-1 top-3 transition-all duration-300"
        />

        {projectData?.projects?.map((pro) => (
          <div
            key={pro.id}
            className="lg:h-[500px] sm:w-[47%] w-full relative md:h-96 h-64 group rounded-3xl overflow-hidden"
          >
            <Image
              src={pro.img}
              alt=""
