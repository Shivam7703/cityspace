  ...
  return (
   <div className="lg:px-28 md:p-20 sm:p-12 p-7 bg-blue1  flex max-md:flex-wrap justify-between gap-y-10 relative">
       ...
       <div
         ...
         className="sm:w-[54%] w-full space-y-8"
       >
         {projectData?.projects && projectData?.projects.map((pro :any)=>(
<div key={pro.id} className="lg:h-[500px] relative md:h-96 h-72 w-full group rounded-3xl overflow-hidden">
<Image src={pro?.img} alt="" className="w-full h-full absolute z-10 top-0 left-0 group-hover:scale-110 duration-300"/>
<div className="absolute z-20 w-full sm:p-14 p-6 flex justify-between text-white items-end h-full bg-gradient-to-t from-[#000000ce] to-transparent top-0 left-0 "><div><h2 className="sm:text-3xl text-2xl">{pro.title}</h2><h4 className="sm:mt-4 mt-2 font-bold text-sm">{pro.text}</h4></div><div><Link href="#" className="flex items-center gap-x-2 group/link">
